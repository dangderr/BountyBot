...
async function check_ping_message(message) {
    ...
    try {
        ...
        if (message.content.includes("is still growing!")) {
            ...
            str = '<@' + user.id + '>' + ' AHHHH DRAAGGGONNNN';
            timestamp.setMilliseconds(timestamp.getMilliseconds() + delay);
            db.set_user_ping_timer(user.id, 'hades_dragon', timestamp.toISOString());
    ...
    for (const ping_timer_row of ping_timer_table) {
        ...
        for (const property in ping_timer_row) {
            ...
            switch (property) {
                ...
                case 'hades_dragon': timer_restart_handler(channel, current_time, ping_timer_row[property], '<@' + discord_id + '>' + ' AHHHH DRAAGGGONNNN', console_message); break;
